...
def RepeatedRandom(A: list, maxIterations: int, S, P, representation = 'standard'):
    n = len(A)
...
def HillClimbing(A: list, maxIterations: int, S, P, representation = 'standard'):
    #n = len(A)
    if (representation == 'standard'):
        # S = GenerateRandomSolution(n)
        bestResidue = GetResidueStandard(A, S)
...
def simAnneal(A: list, maxIterations: int, S, P, representation = 'standard'):
    #n = len(A)
    if (representation == 'standard'):
        #S = GenerateRandomSolution(n)
        bestResidue = GetResidueStandard(A, S)
...
def GenRandomProblem(n, dim):
    return [random.randint(1, n) for _ in range(dim)]

# Input: size of the solution n
# Output: Random pre-partition of size n with S[i] = 1, 2, ..., n (subset an element belongs to)
def GenerateRandomPrePart(dim)->list:
